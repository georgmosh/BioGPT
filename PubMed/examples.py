...
from transformers import pipeline, set_seed, BioGptTokenizer, BioGptForCausalLM, LlamaTokenizer, AutoModelForCausalLM
...
if __name__ == "__main__":
    ...
    model_generation = model.generate(inputs, max_length=100)
    output = tokenizer.decode(model_generation[0])
    print('BioGPT PubMedQA: ', output)

    tokenizer = LlamaTokenizer.from_pretrained('chaoyi-wu/PMC_LLAMA_7B_10_epoch')
    model = AutoModelForCausalLM.from_pretrained("chaoyi-wu/PMC_LLAMA_7B_10_epoch").half().to(device)
    inputs = tokenizer('What is the Effect of chloroquine on cultured fibroblasts?', return_tensors="pt", add_special_tokens=False).to(device)
    with torch.no_grad():
        model_generation = model.generate(inputs=inputs["input_ids"], max_length=200)  #, do_sample=True, top_k=50
        output = tokenizer.decode(model_generation[0])
        print('LlaMA: ', output)
